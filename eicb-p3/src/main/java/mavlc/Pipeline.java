...
import mavlc.codegen.tam.CodeGenerator;
import mavlc.codegen.tam.TamAssembler;
import mtam.Image;
import mtam.errors.ExecutionException;
import mtam.interpreter.Interpreter;
...
public class Pipeline {
	...
	protected final AccessControlContext sandbox;
	...
	protected void compileProgramImpl() {
		TamAssembler asm = new TamAssembler();
		AccessController.doPrivileged((PrivilegedAction<Void>) () -> {
	...
	protected void executeProgramImpl(InputStream in, OutputStream out, int maxCycles) {
		interpreter = new Interpreter(in, new PrintStream(out));
		interpreter.basePath = sourcePath.getParent();
		interpreter.loadImage(image);
	...
	public boolean dumpImage(Path outPath) {
		if(stage.value < Stage.codegen.value)
			throw new InternalCompilerError("Cannot dump image: No program has been compiled yet");
		try {
			image.save(outPath);
			return true;
		} catch(Exception e) {
	...
	public boolean dumpSymbols(Path outPath) {
		if(stage.value < Stage.codegen.value)
			throw new InternalCompilerError("Cannot dump debug symbols: No program has been compiled yet");
		try {
			image.saveSymbols(outPath);
			return true;
		} catch(Exception e) {
			error = e;
			return false;
	...
	public String getDisasm() {
		if(stage.value < Stage.codegen.value)
			throw new InternalCompilerError("Cannot dump disassembly: No program has been compiled yet");
		try {
			return image.getDisassemblyText();
	...
	public boolean dumpDisasm(Path outPath) {
		try {
			FileUtils.write(outPath.toFile(), getDisasm());
			return true;
		} catch(Exception e) {
			error = e;
			return false;
	...
	public boolean dumpOutput(Path outPath) {
		if(stage.value < Stage.execute.value)
			throw new InternalCompilerError("Cannot dump execution output: No program has been executed yet");
		try {
			FileUtils.write(outPath.toFile(), output);
			return true;
		} catch(Exception e) {
			error = e;
			return false;
	...
	public boolean dumpProfile(Path outPath) {
		try {
			FileUtils.write(outPath.toFile(), getProfile());
			return true;
		} catch(Exception e) {
			error = e;
			return false;
	...
	public boolean dumpProfile(OutputStream out) {
		try {
			out.write(getProfile().getBytes(StandardCharsets.UTF_8));
			return true;
		} catch(Exception e) {
			error = e;
			return false;
	...
	private String getProfile() {
		if(stage.value < Stage.execute.value)
			throw new InternalCompilerError("Cannot dump execution profile: No program has been executed yet");
		return interpreter.stats.generateReport(true);
	}
